...
/// (`CGImage` is Apple-only). On macOS the bytes are produced by CoreGraphics
/// draws and wrapped straight back into a `CGImage` for Vision, so the
/// registration input is byte-identical to a direct CGImage decode; on Linux
