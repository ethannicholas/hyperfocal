...
- **GPU fusion on Linux.** The Windows package ships it (2026-08-06); Linux
  ...
  be, it must do what `Scripts/package-windows.ps1` now does: stage
  `libwgpu_native.so` beside the binary, ship
...
1. **Microsoft Store / MSIX submission.** `Scripts/package-windows.ps1` now
   ...
   **unsigned** — Partner Center signs Store submissions, so no code-signing
   ...
   The package ships **GPU fusion** as of 2026-08-06: every Windows build
   compiles it in (Package.swift), and `Scripts/package-windows.ps1` stages
