...
- **GPU fusion on Linux.** The Windows package ships it (2026-08-06); Linux
  ...
  to put it in yet, so nothing to flip. Whatever that packaging turns out to
  be, it must do what `Scripts/package-windows.ps1` now does: build with
  `HYPERFOCAL_WGPU=1`, stage `libwgpu_native.so` beside the binary, ship
  `licenses/wgpu-native-ThirdPartyNotices.txt`, and assert the library is
  actually there rather than silently reverting to CPU fusion.
...
1. **Microsoft Store / MSIX submission.** `Scripts/package-windows.ps1` now
   ...
   **unsigned** — Partner Center signs Store submissions, so no code-signing
   ...
   The package ships **GPU fusion** as of 2026-08-06:
   `Scripts/package-windows.ps1` builds with `HYPERFOCAL_WGPU=1`, stages
   `wgpu_native.dll`, and fails the build if either the wgpu tree or the staged
   DLL is missing — a release must not quietly revert to CPU fusion. Its
   attribution (`licenses/wgpu-native-ThirdPartyNotices.txt`, ~140 Rust crates,
   all permissive) is generated by `Scripts/gen-wgpu-notices.py` and gated on
   the pinned tag.
