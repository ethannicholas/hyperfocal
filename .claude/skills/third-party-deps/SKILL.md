...
- **LGPL / Qt-class:** ensure **dynamic** linking; add the "prominent notice"
  ...
  `QtShell/Main.qml`); and follow the packaging checklist (bundle GPL+LGPL
  texts, host exact source or written offer, don't ship GPL-only build
  tools). The relinking obligation
...
- **Build wiring, everywhere it's referenced:** `Package.swift` (Windows
  ...
  **and** `.github/workflows/ci.yml`. Keep them consistent — a
...
`App/Sources/HyperfocalAppMain.swift` and `QtShell/Main.qml`.
