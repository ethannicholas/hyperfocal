...
if [ "$QT" = 1 ]; then
    ...
    # not opted into — see Package.swift), and there is no rpath, so the
    # loader needs the directory or the process dies at start with a missing-
    # library error before any of our code can report it. macOS links Metal
    # instead and needs none of this. Scripts/windows-env.ps1 does the same
    # job for wgpu_native.dll on Windows.
